...
5. Add a separate Base58 Solana receiving address before enabling the Solana option.
6. Apply D1 migrations, deploy, and test health, config, quote, upload, verification, moderation, WebSocket and refund-queue paths.
7. Independently verify every displayed token contract and receiving address on each network.
8. Turn `PAYMENTS_LIVE=true` only after a small-value mainnet transfer completes the full flow on each enabled network.
