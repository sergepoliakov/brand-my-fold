...
function WaitlistForm({ lang, runtime, standalone = false }) {
  ...
  const submit = async (event) => {
    ...
    if (runtime.offline) {
      if (IS_PRODUCTION_EDITION) { setError(t.unavailable); return; }
      let existing = []; try { existing = JSON.parse(localStorage.getItem(WAITLIST_KEY) || "[]"); } catch { existing = []; }
      localStorage.setItem(WAITLIST_KEY, JSON.stringify([...existing, { email, handle, createdAt: Date.now() }])); setDone(true); return;
    }
  ...
  return <form className={`waitlist-form ${standalone ? "standalone" : ""}`} onSubmit={submit}><input aria-label={t.email} type="email" placeholder={t.email} value={email} onChange={(event) => setEmail(event.target.value)} required/><input aria-label={t.handle} placeholder={t.handle} value={handle} onChange={(event) => setHandle(event.target.value)}/><button disabled={Boolean(IS_PRODUCTION_EDITION && runtime.offline) || Boolean(runtime.turnstileSiteKey && !turnstileToken)}>{t.button}</button><TurnstileChallenge siteKey={runtime.turnstileSiteKey} onToken={setTurnstileToken}/><p>{runtime.offline ? (IS_PRODUCTION_EDITION ? t.unavailable : t.localDemo) : t.local}{error && ` · ${error}`}</p></form>;
}
...
function HomePage({ auction, setAuction, runtime, visitors, onNavigate, lang, setLang, variant, recordEvent }) {
  ...
  const total = auction.spots.filter((spot) => spot.bids > 0).reduce((sum, spot) => sum + Number(spot.amount || 0), 0); const progress = Math.round(total / TARGET_USDT * 100); const headlineAmount = total > 0 ? total : Math.min(...auction.spots.map((spot) => Number(spot.amount || 0))); const headlineLabel = total > 0 ? t.hero.raised : t.hero.starting;
