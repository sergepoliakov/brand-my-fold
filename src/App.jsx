...
import { ArrowLeft, ArrowRight, Check, Copy, ExternalLink, GitFork, Languages, Plus, ShieldCheck, Upload, X } from "lucide-react";
import { siCloudflare, siFigma, siFramer, siGithub, siLinear, siNotion, siRaycast, siStripe, siSupabase, siVercel } from "simple-icons";
import { CHANNEL_NAME, COPY, IS_PRODUCTION_EDITION, LANGUAGE_KEY, PRODUCT_IMAGES, PRODUCT_NAME, seedSpots, STORAGE_KEY, TARGET_USDT, TRAFFIC_SESSION_KEY } from "./content.js";
import { createBidQuote, loadAuction, loadRuntimeConfig, loadTraffic, recordExperiment, recordTrafficHeartbeat, subscribeToAuction, uploadArtwork, verifyBidPayment } from "./api.js";
...
const BRAND_ICONS = { cloudflare: siCloudflare, figma: siFigma, framer: siFramer, github: siGithub, linear: siLinear, notion: siNotion, raycast: siRaycast, stripe: siStripe, supabase: siSupabase, vercel: siVercel };
...
function BrandMark({ spot, label = false }) {
  ...
  const brand = spot.brand || spot.previewBrand;
  const icon = !spot.brand ? BRAND_ICONS[spot.previewIcon] : null;
  const color = spot.brand ? spot.color : spot.previewColor;
  if (!brand) return <span className="brand-available">{spot.coordinate}</span>;
  return <span className={`brand-lockup ${icon ? "brand-showcase" : ""}`} style={{ color: `#${color || "1d1d1f"}` }}>{icon ? <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d={icon.path}/></svg> : <span className="brand-fallback">{brand.slice(0, 2).toUpperCase()}</span>}{label && <span>{brand}</span>}</span>;
}
...
function PresentationControl({ value, onChange, lang }) {
  ...
  return <div className="presentation-control" aria-label={lang === "zh" ? "设备展示状态" : "Device presentation state"}><button aria-pressed={value === "unfolded"} onClick={() => onChange("unfolded")}>{t.unfolded}</button><button aria-pressed={value === "folded"} onClick={() => onChange("folded")}>{t.folded}</button></div>;
}

function DeviceStage({ spots, presentation, onSelect, lang }) {
  const t = COPY[lang];
  return <div className={`device-stage device-stage-${presentation}`}><img className="device-render active" src={PRODUCT_IMAGES[presentation]} alt={`${PRODUCT_NAME} · ${presentation}`}/><span className="surface-zone-label">{t.hero.printable}</span>{presentation === "unfolded" && <span className="screen-zone-label"><i/>{t.hero.screen}</span>}{spots.map((spot) => {
    const [left, top, width, height] = spot.positions[presentation];
    return <button key={spot.id} className="surface-spot" style={{ left: `${left}%`, top: `${top}%`, width: `${width}%`, height: `${height}%` }} onClick={() => onSelect(spot)} aria-label={`${spot.coordinate} · ${lang === "zh" ? spot.nameZh : spot.name} · ${formatMoney(spot.amount, lang)}`}><span className="spot-id">{spot.id}</span><span className="spot-content"><BrandMark spot={spot} label={spot.size !== "S"}/><span className="spot-price">{formatMoney(spot.amount, lang)}</span></span><span className="spot-hover">{spot.bids ? t.hero.outbid : t.hero.bid}</span></button>;
  })}</div>;
}
...
function BidModal({ spot, runtime, onClose, onDemoBid, onVerifiedBid, lang, onQuoteCreated }) {
  ...
  const productionUnavailable = IS_PRODUCTION_EDITION && !paymentsEnabled;
  ...
  const submitDetails = async (event) => {
    ...
    if (!paymentsEnabled) {
      if (productionUnavailable) { setError(t.modal.unavailable); return; }
      onDemoBid({ ...spot, amount: Number(bid), bids: spot.bids + 1, brand: brand.trim(), email, url: website.trim(), handle: handle.trim(), customLogo: preview, color: "1d1d1f" });
      return;
    }
...
function AuctionTable({ spots, onSelect, lang }) {
  const t = COPY[lang];
  const ranked = [...spots].sort((left, right) => Number(right.amount) - Number(left.amount) || Number(right.bids) - Number(left.bids) || left.id - right.id);
  return <div className="auction-table-wrap"><table className="auction-table"><thead><tr><th>{t.auction.placement}</th><th>{t.auction.size}</th><th>{t.auction.held}</th><th>{t.auction.bid}</th><th><span className="sr-only">{t.auction.action}</span></th></tr></thead><tbody>{ranked.map((spot) => <tr key={spot.id}><td><span className="spot-number">{spot.id}</span><span>{lang === "zh" ? spot.nameZh : spot.name}<small>{spot.coordinate} · {t.auction.shell}</small></span></td><td><span className="size-badge">{spot.size}</span><small>{spot.dimensions}</small></td><td>{spot.brand ? (spot.url ? <a href={spot.url} target="_blank" rel="noreferrer"><BrandMark spot={spot} label/></a> : <BrandMark spot={spot} label/>) : <span className="available-cell">{t.auction.available}</span>}</td><td><strong>{formatMoney(spot.amount, lang)}</strong><small>{spot.bids} {spot.bids === 1 ? t.auction.oneBid : t.auction.bids}</small></td><td><button className="table-bid" onClick={() => onSelect(spot)}>{t.auction.action}</button></td></tr>)}</tbody></table></div>;
}

function Footer({ onNavigate, lang }) {
  const t = COPY[lang].footer;
  return <footer className="site-footer"><div className="footer-inner"><div className="founder-card"><div className="founder-avatar">S</div><div><p><strong>{t.hi}</strong></p><p>{t.bio}</p></div></div><p>{t.prompt} <a href="https://github.com/sergepoliakov/brand-my-fold" target="_blank" rel="noreferrer">{t.join}</a></p><nav><a href="/privacy" onClick={(event) => { event.preventDefault(); onNavigate("/privacy"); }}>{t.privacy}</a><a href="/terms" onClick={(event) => { event.preventDefault(); onNavigate("/terms"); }}>{t.terms}</a><a href="https://github.com/sergepoliakov/brand-my-fold" target="_blank" rel="noreferrer">{t.source}<ExternalLink size={10}/></a></nav><p className="legal-small">{t.legal}</p></div></footer>;
}

function HomePage({ auction, setAuction, runtime, traffic, onNavigate, lang, setLang, variant, recordEvent }) {
  const t = COPY[lang];
  const [presentation, setPresentation] = useState("unfolded"); const [selected, setSelected] = useState(null); const [toast, setToast] = useState(""); const [, setNow] = useState(Date.now());
  useEffect(() => { const timer = window.setInterval(() => setNow(Date.now()), 30000); return () => window.clearInterval(timer); }, []);
  const total = auction.spots.filter((spot) => spot.bids > 0).reduce((sum, spot) => sum + Number(spot.amount || 0), 0); const progress = Math.round(total / TARGET_USDT * 100); const held = auction.spots.filter((spot) => spot.bids > 0).length;
...
export function App() {
  const [auction, setAuction] = useState(readLocalAuction); const [runtime, setRuntime] = useState({ edition: IS_PRODUCTION_EDITION ? "production" : "open-source", paymentsLive: false, networks: [], auctionCurrency: "USDT", offline: true, failClosed: IS_PRODUCTION_EDITION }); const [lang, setLang] = useState(() => localStorage.getItem(LANGUAGE_KEY) || "zh"); const [route, setRoute] = useState(window.location.pathname);
  const [traffic, setTraffic] = useState({ liveVisitors: 0, totalVisitors: 0, visits: [], countries: [], sources: [], updatedAt: "" });
  const [trafficSession] = useState(() => { let value = sessionStorage.getItem(TRAFFIC_SESSION_KEY); if (!value) { value = crypto.randomUUID(); sessionStorage.setItem(TRAFFIC_SESSION_KEY, value); } return value; });
  const [trafficContext] = useState(() => { let referrerHost = ""; try { const referrer = document.referrer ? new URL(document.referrer) : null; if (referrer && referrer.origin !== window.location.origin) referrerHost = referrer.hostname; } catch { referrerHost = ""; } return { referrerHost, utmSource: new URLSearchParams(window.location.search).get("utm_source") || "" }; });
  ...
  useEffect(() => {
    if (runtime.offline) return undefined;
    let active = true;
    const heartbeat = async () => { try { const result = await recordTrafficHeartbeat({ sessionId: trafficSession, ...trafficContext }); if (active && result.traffic) setTraffic(result.traffic); } catch { /* Keep the last verified traffic snapshot. */ } };
    const refresh = async () => { try { const result = await loadTraffic(); if (active) setTraffic(result); } catch { /* Heartbeats retry automatically. */ } };
    heartbeat();
    const heartbeatTimer = window.setInterval(heartbeat, 30000);
    const refreshTimer = window.setInterval(refresh, 15000);
    return () => { active = false; window.clearInterval(heartbeatTimer); window.clearInterval(refreshTimer); };
  }, [runtime.offline, trafficSession, trafficContext]);
  ...
  const cta = ctaCopy(lang, variant);
  const page = useMemo(() => route === "/privacy" || route === "/terms" ? <LegalPage type={route.slice(1)} onNavigate={navigate} lang={lang} setLang={setLang} cta={cta}/> : <HomePage auction={auction} setAuction={setAuction} runtime={runtime} traffic={traffic} onNavigate={navigate} lang={lang} setLang={setLang} variant={variant} recordEvent={recordEvent}/>, [route, auction, runtime, traffic, lang, variant, anonymous]);
